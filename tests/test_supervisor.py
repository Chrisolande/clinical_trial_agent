...
async def test_supervisor_calls_subagents_in_sequence(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    class DummyMemory:
        ...
        async def list_feedback(self, profile_hash: str | dict[str, Any]) -> list[dict[str, Any]]:
            _ = profile_hash
            return []
...
@pytest.mark.asyncio
async def test_supervisor_feedback_lookup_uses_patient_profile_for_scoped_hash(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    orchestrator = SupervisorOrchestrator()
    patient_profile = {"age": 40, "primary_condition": "synthetic lung cancer"}
    seen_lookup_payloads: list[Any] = []
    synthesis_scores: list[list[dict[str, Any]]] = []
    ...
    async def fake_eligibility(**_: Any) -> dict[str, Any]:
        return {
            "trial_scores": [
                {"trial_id": "NCT1", "tier": "moderate", "score": 0.60},
                {"trial_id": "NCT2", "tier": "moderate", "score": 0.59},
            ],
            "eligibility_verdicts": {},
            "missing_info_recommendations": [],
            "decision_history": [],
    ...
    async def fake_synthesis(**kwargs: Any) -> dict[str, Any]:
        synthesis_scores.append(list(kwargs["trial_scores"]))
        return {"report_json": {"ranked_trials": kwargs["trial_scores"]}, "report_text": "done"}

    class DummyMemory:
        async def list_feedback(self, profile_hash: str | dict[str, Any]) -> list[dict[str, Any]]:
            seen_lookup_payloads.append(profile_hash)
            return [{"nct_id": "NCT2", "verdict": "confirmed"}]
