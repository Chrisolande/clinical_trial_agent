...
_NO_CONCERN_PHRASES = ("no concerns", "no major concerns", "none identified", "no significant concerns")
...
def _check_public_report_internal_leakage(
    scored_trials: list[dict[str, Any]], issue_builder: Callable[[str, str, str], dict[str, str]]
) -> list[dict[str, str]]:
    flagged: list[str] = []
    for trial in scored_trials:
        lowered_snippets = [snippet.lower() for snippet in _trial_text_snippets(trial)]
        if any(phrase in snippet for snippet in lowered_snippets for phrase in _INTERNAL_LEAKAGE_PHRASES):
            flagged.append(str(trial.get("trial_id", "")))
    if not flagged:
        return []
    return [issue_builder("PUBLIC_REPORT_INTERNAL_LEAKAGE", "critical", f"Internal processing language leaked into report-facing narrative in trial(s): {flagged[:5]}.")]
...
def _check_strong_too_few_criteria(
    ...
    if not flagged:
        return []
    return [issue_builder("STRONG_TOO_FEW_CRITERIA", "high", f"Strong-tier trial(s) have too few assessed criteria (<3), which may overstate confidence: {flagged[:5]}.")]
...
def _check_strong_match_contradiction(
    ...
    if not flagged:
        return []
    return [issue_builder("STRONG_MATCH_CONTRADICTION", "high", f"Strong match narrative overstates certainty despite unresolved uncertainty or gaps in trial(s): {flagged[:5]}.")]


def _check_na_inference_errors(
    eligibility_verdicts: dict[str, dict[str, Any]], issue_builder: Callable[[str, str, str], dict[str, str]]
) -> list[dict[str, str]]:
    flagged: list[str] = []
    ...
    if not flagged:
        return []
    return [issue_builder("N_A_INFERENCE_ERROR", "high", f"Criteria marked as not applicable appear to have been inferred as MEETS/FAILS in trial(s): {flagged[:5]}.")]


def _missing_info_key(item: Any) -> str:
    if isinstance(item, dict):
        return str(item.get("field_id") or item.get("field") or item.get("display_name") or "").strip().lower()
...
def _check_duplicate_missing_info(
    ...
    if not trials_with_duplicates:
        return []
    return [issue_builder("DUPLICATE_MISSING_INFO", "medium", f"Duplicate critical_missing_info entries detected in trial(s): {trials_with_duplicates[:5]}. Deduplicate before synthesis.")]
...
def _check_not_applicable_leakage(
    ...
    if not flagged:
        return []
    return [issue_builder("N_A_LEAKAGE_IN_GAPS", "high", f"Not-applicable items leaked into critical_missing_info in trial(s): {flagged[:5]}. Suppress these from information gaps and actions.")]
...
def _check_generic_biomarker_inference(
    ...
    flagged: list[str] = []
    for trial in scored_trials:
        texts = [str(trial.get("key_concern", "")), str(trial.get("rationale", "")), *[str(item) for item in trial.get("critical_missing_info", []) or []]]
        for text in texts:
            lowered = text.lower()
    ...
    if not flagged:
        return []
    return [issue_builder("GENERIC_BIOMARKER_INFERENCE", "high", f"Generic biomarker language without specific marker detected in trial(s): {flagged[:5]}. Use marker-specific evidence where possible.")]
...
def _check_summary_language_vs_critical_gaps(
    ...
    flagged: list[str] = []
    for trial in scored_trials:
        missing = [str(item).strip() for item in trial.get("critical_missing_info", []) or [] if str(item).strip()]
        if not missing:
            continue
        narrative = " ".join([str(trial.get("key_concern", "")), str(trial.get("rationale", ""))]).lower()
        if any(phrase in narrative for phrase in _NO_CONCERN_PHRASES):
            flagged.append(str(trial.get("trial_id", "")))
    if not flagged:
        return []
    return [issue_builder("EXEC_SUMMARY_CONTRADICTS_CRITICAL_GAPS", "high", f"Reassuring language conflicts with critical missing information in trial narrative(s): {flagged[:5]}.")]
...
def _check_low_priority_gap_bloat(
        ...
        for item in trial.get("critical_missing_info", []) or []:
            if isinstance(item, dict):
                ...
                if priority == "low":
                    ...
                    continue
                descriptor = " ".join(str(item.get(key, "")) for key in ("field_id", "field", "description")).lower()
    ...
    if not flagged:
        return []
    return [issue_builder("LOW_PRIORITY_GAP_BLOAT", "medium", f"Low-priority information gaps are excessively enumerated in trial(s): {flagged[:5]}. Consolidate low-priority gaps.")]


def check_additional_quality_rules(
    eligibility_verdicts: dict[str, dict[str, Any]],
    scored_trials: list[dict[str, Any]],
    issue_builder: Callable[[str, str, str], dict[str, str]],
) -> list[dict[str, str]]:
